...
export function Navbar({ session }) {
  ...
  const navLinks = () => {
    ...
    if (isAuthenticated) {
      ...
      links.push({
        href: role === "student" ? "/student/wallet" : "/client/wallet",
        label: "Wallet",
      });
      if (role === "student") {
  ...
  return (
    <header className="bg-background w-full flex justify-center py-6 border-b border-border/10">
                    ...
                    {isAuthenticated ? (
                      <>
                        ...
                        <form action={logout}>
                          <button
                            type="submit"
                            className="w-full text-left text-[15px] font-medium text-red-600 hover:text-red-700"
                          >
                            Sign Out
                          </button>
                        </form>
                      ...
                      <>
                        <Link
                          href="/login"
                          className="text-[15px] font-medium text-[#181717]"
                        >
                          Sign In
                        </Link>
                        <Link
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <form action={logout}>
                    <button
                      type="submit"
                      className="text-[15px] font-medium text-[#181717] hover:text-primary"
                    >
                      Sign Out
                    </button>
                  </form>
