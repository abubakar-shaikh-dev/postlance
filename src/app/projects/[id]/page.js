...
export default async function ProjectDetailPage({ params }) {
  ...
  if (session?.role === 'student') {
    ...
    hasApplied = proposals.some(p => (p.projectId?._id ? p.projectId._id.toString() : p.projectId?.toString()) === project._id.toString());
  }
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-between items-start mb-10 pb-8 border-b border-border/10">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 rounded text-[13px] font-medium tracking-wide ${statusStyles[project.status]}`}>
          ...
          <div className="flex items-center gap-3 mt-4 text-[#666666]">
            <Avvvatars
              value={project.clientId?.email || project.clientId?.name || project.clientId?._id || '?'}
              size={32}
            />
            <span className="text-[15px] font-medium">{project.clientId?.name || 'Unknown Client'}</span>
          </div>
        </div>
        ...
        <div className="flex shrink-0 gap-3 w-full md:w-auto">
          ...
          {session?.role === 'student' && !isAcceptingProposals && project.status === 'open' && !hasApplied && (
            <button
              disabled
          ...
          {session?.userId === project.clientId?._id && (
            <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
              <StatusDropdown projectId={project._id} currentStatus={project.status} />
              <Link
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-8">
          <div className="bg-white rounded-[32px] p-8 md:p-10 border border-border/10">
            <h2 className="text-[24px] font-medium text-[#181717] mb-6">Description</h2>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[32px] p-8 border border-border/10">
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[14px] text-[#666666] font-medium mb-1">Budget</p>
                  <p className="font-medium text-[20px] text-[#181717]">{project.budget}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[14px] text-[#666666] font-medium mb-1">Deadline</p>
                  <p className="font-medium text-[16px] text-[#181717]">
                    {deadline.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[14px] text-[#666666] font-medium mb-1">Posted On</p>
                  <p className="font-medium text-[16px] text-[#181717]">
                    {new Date(project.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}
                  </p>
                </div>
              </div>
            </div>
          ...
          {project.agreedAmount != null && (
            <div className="bg-white rounded-[32px] p-8 border border-border/10">
              <h3 className="text-[20px] font-medium text-[#181717] mb-6">Payment Progress</h3>
              {(() => {
                ...
                return (
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[13px] text-[#666666] font-medium">Hired Student</p>
                    ...
                    <div className="flex justify-between text-[14px]">
                      ...
                      <span className="font-medium text-[#181717]">₹{agreed.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-[14px]">
                      <span className="text-[#666666]">Paid</span>
                      <span className="font-medium text-[#181717]">₹{paid.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-[14px]">
                      <span className="text-[#666666]">Remaining</span>
                      <span className={`font-medium ${isFullyPaid ? 'text-[#cbf4c9]' : 'text-[#d04841]'}`}>
                        {isFullyPaid ? 'Fully Paid' : `₹${remaining.toLocaleString()}`}
                      </span>
                    </div>
                    ...
                    <div className="h-2 bg-[#eeebea] rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          isFullyPaid ? 'bg-[#cbf4c9]' : progress > 0 ? 'bg-[#d04841]' : 'bg-[#eeebea]'
                        }`}
                        style={{ width: `${Math.min(progress, 100)}%` }}
                      />
                    </div>

                    {session?.userId === project.clientId?._id && !isFullyPaid && (
                      <Link
                        href="/client/wallet"
          ...
          <div className="bg-white rounded-[32px] p-8 border border-border/10">
            <h3 className="text-[20px] font-medium text-[#181717] mb-6">Skills Required</h3>
            <div className="flex flex-wrap gap-2">
              {project.skillsRequired?.map((skill) => (
                <span key={skill} className="px-3 py-1.5 rounded-lg bg-[#eeebea] text-[#181717] text-[14px] font-medium">
                  {skill}
                </span>
              ))}
            </div>
